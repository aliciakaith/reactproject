import { benefits } from "../constants";
import Heading from "./Heading";
import Section from "./Section";
import Arrow from '../assets/svg/Arrow';
import {GradientLight} from './design/Benefits';
import ClipPath from "../assets/svg/ClipPath";

const Benefits = () => {
  return (
    <Section id="features">
        <div className="container relative z-2">
            <Heading className="md:max-w-md lg:max-w-2xl" title="Chat Smarter, Not Harder with Brainwave"/>
            <div className="flex flex-wrap gap-10 mb-10">
                {benefits.map((item) => ( /* item is the name given to each benefit from constants/index.js, so we can access properties of each benefit using item.propertyName */
                    <div className="block relative p-0.5 bg-no-repeat bg-[length:100%_100%] md:max-w-[24rem]"
                    style={{ backgroundImage: `url(${item.backgroundUrl})`,}} key={item.id}>
                        <div className="relative z-2 flex flex-col min-h-[22rem] p-[2.4rem] pointer-events-none" >
                            <h5 className="h5 mb-5">{item.title}</h5>
                            <p className="body-2 mb-6 text-n-3">{item.text}</p>
                            <div className="flex items-center mt-auto">
                                <img src={item.iconUrl} width={48} height={48} alt={item.title}/>
                                <p className="ml-auto font-code text-xs font-bold text-n-1 uppercase tracking-wider">Explore more</p>
                                <Arrow />
                            </div>
                        </div>
                        {/* if item light is true (can see from constants/index.js), then render GradientLight component, else render nothing */}
                        {item.light && <GradientLight />}

                        <div className="absolute inset-0.5 bg-n-8" style={{ ClipPath: "url(#benefits)" }}>
                            {/* inset means to set all four sides (top, right, bottom, left) to 0, so the div stretches to fill its parent completely
                            and this only works because the parent has relative positioning.*/ }
                            {/* transition-opacity means that when the opacity changes, it will transition smoothly instead of changing abruptly */}
                            <div className="absolute inset-0 opacity-0 transition-opacity hover:opacity-10">
                                {/* if item has an image URL, display the image url*/}
                                {item.imageUrl && (<img src={item.imageUrl} width={380} height={362} alt={item.title} className="w-full h-full object-cover"/>)}
                            </div>
                        </div>
                        <ClipPath />
                    </div>
                ))}
            </div>
        </div>
    </Section>
  )
}

export default Benefits

