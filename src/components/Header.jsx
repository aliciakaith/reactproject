...
const Header = () => {
    ...
    {/* pathname is used to get the current url hash, so that we can highlight the current page in the navigation */}
    ...
    const toggleNavigation = () => {
        ...
        {/* if openNavigation is true, then disable page scroll, else enable page scroll */}
    };
    ...
    return (
      ...
      ${openNavigation ? "bg-n-8" : "bg-n-8/90 backdrp-blur-sm"}`}>
        <div className="flex items-center px-5 lg:px-7.5 xl:px-10 max-lg:py-4">
            ...
            <nav className={`${openNavigation ? 'flex' : 'hidden'} fixed top-[5rem] left-0 right-0 bottom-0 bg-n-8 lg:static lg:flex lg:mx-auto lg:bg-transparent`}>
                <div className="relative z-2 flex flex-col items-center justify-center m-auto lg:flex-row">
                    {/* for each item in navigation, render a link and the item title */}
                    {navigation.map((item) => (
