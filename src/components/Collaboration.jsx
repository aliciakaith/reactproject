            ...
            <div className="relative left-1/2 flex w-[22rem] aspect-square border border-n-6 rounded-full -translate-x-1/2 scale:75 md:scale-100">
                ...
                <ul>
                    {collabApps.map((app, index) => (
                        <li key={app.id} className={`absolute top-0 left-1/2 h-1/2 -ml-[1.6rem] origin-bottom rotate-${index * 45}`}> {/* rotate each app by 45 degrees in the upper div and then rotate it back by the same amount in the inner div, so that the app icons are not rotated and are facing upright */}
